...
# Workshop 1: Introduction to R {#sec-workshop01}
...
3.  Employ variables to store the results of analyses

4.  Describe the different data types and data structures used in R

5.  Compose R code that uses subsetting to operate on vectors and data frames
:::

## Getting started {#sec-starting}
...
![The project folder contains a 'data' sub-folder and an R project file](images/S01F00_rproject.png){width="960"}
...
Remember that this file is called an **R Script**. Don't forget to save your R Script as you work so you don't lose your progress! You can do this through the file menu or by using the keyboard shortcut {{< kbd Cmd-S >}}.

### Writing our first piece of code

Now we are ready to write some code! You might like to start with some of what we learned last week, in section @sec-maths of the ebook.
...
1.  `10 + 0 # + 60` is equal to 10, because the + 60 is after the comment symbol `#`. If you're not sure, try copy-pasting this code into the console and running it! The best way to learn is by doing.
...
This is really useful for filtering data, which we will cover in [Session @sec-workshop02]
...
Variables are assigned with the assignment operator `<-` (you can type this using the {{< kbd < >}} and {{< kbd - >}} keys, or use the shortcut {{< kbd mac=Option- win=alt- >}}).
...
#### Subsetting vectors {#sec-vectorsubset}
...
This returns `"e"`, the fifth letter of the alphabet. Using the colon notation described above, we can also extract a range of positions from the vector, for example:
...
It should be noted that, unlike some other programming languages R uses **1-based indexing**. So that means that the first letter of the alphabet, 'a' can be accessed using `letters[1]`. The indexing is also inclusive, so 10:15 includes the 15th position.
...
The way negative number indexing works in R is a little different to other languages also! If in doubt, just play around with the indexing, worst case scenario you have to try a few different expressions before you land on the correct one.
...
### Matrices {#sec-matrixbrief}

A matrix is the two-dimensional extension of the vector-- it stores a collection of values **of the same type** that are laid out in a grid with rows and columns. In this subject, we will only really use matrices in the context of RNA-seq analysis, so we will cover them then, or you can consult the further reading @sec-matrices.
...
Finally, data frames are **mutable**, meaning that you can add additional columns to them. You can do this by specifying your new column name with the `dataset$column_name` notation, and then assigning a vector into it, like `dataset$new_column_name <- new_column_contents`.
...
Be careful that, as we discussed above every column in a dataframe must have the same length. For example, if I tried to run the following:
...
I get an error, because iris has 150 rows (so each column is a vector of length 150), and I'm trying to add to it a column with length 100.
...
{{< downloadthis resources/session_1_summary_sheet.pdf dname="session_1_summary_sheet" label="Download the summary sheet for workshop 1" icon="file-earmark-arrow-down" type="primary" class="data-button" id="summarysheet" >}}
...
5.  I'm working with the `iris` data frame, but I feel like it needs to be more dramatic. I would like to create a new data frame called `IRIS` where the species in the `Species` column are all in capital letters. How can I do this? (hint: the function `toupper()` might help you out here)
...
1.  The results of evaluating the expressions are:
    ...
    o.  `iris$Sepal.Width[1:5]` will give the first 5 values of the `Sepal.Width` column from the `iris` dataset.
...
5.  First, we would want to assign (i.e. copy) the contents of the `iris` data frame into our new `IRIS` data frame. Then, we can use the `toupper()` function on the `Species` column like so:

```{r}
# make a new variable called IRIS that contains the 
