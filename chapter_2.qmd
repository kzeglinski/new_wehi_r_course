...
Before we start, let's use what we learned in the previous chapter to take a look at `mousezempic_dosage_data`:
...
::: question
1\. Which line of code would NOT be a valid way to select only the `drug_dose_g`, `initial_weight_g`, and `weight_lost_g` columns from the `mousezempic_dosage_data` data frame?
