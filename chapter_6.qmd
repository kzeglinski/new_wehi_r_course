...
We will 1.
extract the summary statistics for all three contrasts into separate tables 2.
extract the sample_ids and experimental group labels, and then 3.
extract the log(CPM) values for each sample

### 1. Extract summary tables
...
### 2. Extract log(counts per million)
...
The information we'll need is in the `top_cat` tibble that we extracted for the volcano plot.
