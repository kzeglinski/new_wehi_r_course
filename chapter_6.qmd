...
In this case we'll make boxplots from the log(CPM) values of individual samples, for the first 6 differentially expressed genes in `top_gene_expr`, which come from the BasalvsLP topTable. 
We will extract the first 6 rows of the tibble using `slice`, then reshape the data to long form as required for ggplot2.

```{r}
top_gene_long <- top_gene_expr |> as_tibble(rownames='SYMBOL') |> 
  slice_head(n=6) |> 
...
Next, we can create a heatmap of the differentially expressed genes. The heatmap is a useful way to visualise the expression of the differentially expressed genes in individual samples, especially when more than 2 conditions are being tested. We can use the `pheatmap` package to create the heatmap.
