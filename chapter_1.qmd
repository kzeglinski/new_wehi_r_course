...
# Chapter 1: Introduction to R {#sec-chapter01}
...
This panel also shows you the names and types of the columns in the data frame. You might notice that the `Species` is described as a factor, which is a special type of data in R that we'll cover in @sec-chapter03.
...
{{< downloadthis data/mousezempic_dosage_data.csv dname="mousezempic_dosage_data" label="Download the example data file" icon="file-earmark-arrow-down" type="primary" class="data-button" id="chapter01_data" >}}
...
To read in our data, we'll use the `read_delim()` function from the `readr` package. This function takes in the path to the file you want to read in (in quotation marks `""`, as this is a character string) and returns as output a tibble (this is basically the same as a data frame _except_ that tibbles can not have row names).
