...
When using `scale_colour_manual()`, you need to specify to the `values` argument a vector of colours that matches the number of levels in the variable you're mapping to colour. In the example above, we have three mouse strains, so we need to provide three colours in the `values` argument.
...
If it doesn't do this automatically, you can turn it on by going to `Tools` > `Global Options` > `Code` > `Display` and checking the "Highlight color names and hex codes" option.

:::

You can also use the `scale_colour_gradient()` function to create a gradient of colours for continuous variables.

There are also a range of pre-defined colour palettes you can use, such as `scale_colour_brewer()` or `scale_colour_viridis()`, and there are countless other colour palettes available in R packages like `ggsci`, `ggthemes` and `RColorBrewer`.
...
For example, if we wanted to limit the x axis of our scatterplot to only show values between 0 and 10, we could use the `xlim()` function:
```{r}
#| warning: false
ggplot(data = mousezempic_dosage_data, aes(x = initial_weight_g, y = tail_length_mm)) +
  geom_point() +
  xlim(0, 10)
```
This will limit the x axis to only show values between 0 and 10, and any data points outside of this range will be removed from the plot. This is useful when you want to focus on a specific range of values, or when you have outliers that are skewing the plot.
You can also use the `ylim()` function to limit the y axis in the same way:
