...
And read in the `mousezempic_dosage_data` that we have been using in previous chapters:
...
::: choices
::: choice
`ggplot(data = mousezempic_dosage_data, aes(x = mouse_strain, y = tail_length_mm)) + geom_violin(geom_jitter(width = 0.2)) `
:::

::: choice
`ggplot(data = mousezempic_dosage_data, aes(x = mouse_strain, y = tail_length_mm)) + geom_jitter(width = 0.2)`
:::
...
2.  The correct way to create a violin plot of the tail length of the mice by strain, with points overlaid, is `ggplot(data = mousezempic_dosage_data, aes(x = mouse_strain, y = tail_length_mm)) + geom_violin() + geom_jitter(width = 0.2)`. This specifies the data and mapping in the `ggplot()` function call, adds a layer of violin plots using `geom_violin()`, and then adds a layer of points using `geom_jitter()` to avoid overlapping.
