...
{{< downloadthis data/mousezempic_dosage_data.csv dname="mousezempic_dosage_data" label="Download the example data file" icon="file-earmark-arrow-down" type="primary" class="data-button" id="session01_data" >}}
...
Packages are collections of functions that other people have written to help us do specific tasks, beyond what is built-in to R itself. The [tidyverse](https://www.tidyverse.org/) is a collection of packages that help to streamline data analysis in R. `readr` is a tidyverse package that helps us to read in data from files. Whenever we want to use functions from a package, we need to load it into our R session first by using the `library()` function.
:::
...
If running this code gives you an error message, you might need to install the `readr` package first. Visit @sec-instPackages of the setup part of this book for instructions on how to install the `tidyverse` package, which includes `readr` and other important packages we will use in this course.
...
#| echo: false
# this is for rendering the book, path is different for learners so don't show this chunk
dosage_data <- read_delim("data/mousezempic_dosage_data.csv")
```
